{
  ...
  hmUser = lib.singleton (hm: let
    ...
    flakePath = myLib.flakePath hm-config;
    getUserVars = opt: myLib.utils.getUserVars opt config;
    EMDOTDIR_VAR = hm-config.home.sessionVariables.EMDOTDIR;
    DOOMDIR_VAR = hm-config.home.sessionVariables.DOOMDIR;
    username = hm-config.home.username;
    full-name = getUserVars "fullName";
    email-address = getUserVars "email";
    pack =
  ...
  in {
    services.emacs = {
      enable = true;
      defaultEditor = lib.mkIf ((getUserVars "defaultEditor") == "doom-emacs") true;
    ...
    xdg.configFile = {
      ...
      "doom/system-vars.el".text = ''
         ...
         (setq user-emacs-directory "${EMDOTDIR_VAR}" ; Path to emacs config folder
               ...
               flake-directory "${flakePath}" ; Path to NixOS Flake
        ...
        ${lib.optionalString hm-config.stylix.enable ''
