...
- [[https://github.com/librephoenix/nixos-config/tree/main/user/app/doom-emacs][librephoenix's nixos flake: doom.org file]] flake - Org-mode agenda configurations
...
;; https://gitlab.com/NovaViper/NixConfig/-/blob/main/home/novaviper/dots/doom/config.org
...
- =all-the-icons=: Add fancy icons into Emacs
...
This tells emacs where I've placed my =.authinfo.gpg= file and sets up password look up for mu4e. For Yubikey usage, the passphrase Emacs prompts for is the yubikey pin!
#+begin_src emacs-lisp
...
(setq org-agenda-category-icon-alist
      `(("personal" ,(list (all-the-icons-material "person" :height 0.9)) nil nil :ascent center)
        ("school" ,(list (all-the-icons-material "school" :height 0.9)) nil nil :ascent center)
...
Forces nix-mode to use alejandra. The =nix-nixfmt-bin= specifically utilizes the wrapper =alejandra-quiet= (which is declared by my NixOS flake) that has alejandra prefixed with the =--quiet= option
#+begin_src emacs-lisp
(after! nix-mode
...
(load (concat doom-user-dir "mu4e-contrib.el"))
#+end_src
