      ...
      configFile = {
        ...
        "doom/system-vars.el".text = ''
          ...
          (setq user-emacs-directory "${config.home.sessionVariables.EMDOTDIR}" ; Path to emacs config folder
                ${lib.optionalString (osConfig.variables.user.fullName != "") ''user-full-name "${osConfig.variables.user.fullName}" ; Name''}
                user-username "${config.home.username}" ; username
                ${lib.optionalString (osConfig.variables.user.emailAddress != "") ''user-mail-address "${osConfig.variables.user.emailAddress}" ; Email''}
        ...
        /*
          ...
          (setq user-emacs-directory "${config.home.sessionVariables.EMDOTDIR}" ; Path to emacs config folder
                ${lib.optionalString (config.fullName != "") ''user-full-name "${config.fullName}" ; Name''}
                user-username "${config.home.username}" ; username
                ${lib.optionalString (config.emailAddress != "") ''user-mail-address "${config.emailAddress}" ; Email''}
                mail_directory "${config.xdg.dataHome}" ; Path to mail directory (for mu4e)
                flake-directory "${config.home.sessionVariables.FLAKE}" ; Path to NixOS Flake
