...
in {
  ...
  nukeFiles = let homeDir = myLib.utils.getHMOption "home.homeDirectory" config; in ["${homeDir}/.config/gtk-2.0/gtkrc" "${homeDir}/.config/gtk-3.0/gtk.css" "${homeDir}/.config/gtk-4.0/gtk.css" "${homeDir}/.gtkrc-2.0"];
  ...
  in
    lib.mkIf config.stylix.enable {
      ...
      */
      ...
      stylix.targets.floorp.profileNames = ["${config.userVars.username}"];
