...
in {
  ...
  users.users.${config.userVars.username}.extraGroups = ["gamemode"];
  ...
  networking.firewall.allowedTCPPorts = mkIf cfgFeat.includeMinecraftServer [25565 24454];
  ...
  programs.steam = {
    ...
    package = pkgs.steam.override {
      extraEnv.HOME = "${hm-config.home.homeDirectory}/.config";
  ...
  hm.home.shellAliases = mkIf cfgFeat.includeMinecraftServer {
